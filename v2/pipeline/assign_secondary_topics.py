"""
Assign primary (if missing) and secondary topics to all questions using
rule-based tag + keyword matching. No LLM / API key required.

Usage:
    cd v2/pipeline
    python3 assign_secondary_topics.py [--dry-run]
"""
...
TAG_TOPIC: dict[str, str] = {
    ...
    "politics": "history", "empire": "history", "colonialism": "history",
    ...
    "fields medal": "science", "mathematics": "science",
...
# ── Per-question rule overrides ───────────────────────────────────────────────
# For questions where heuristics alone can't reliably assign a topic.
# Key: substring of question text (lowercase), value: (primary, secondary|None)

TEXT_OVERRIDES: list[tuple[str, str, str | None]] = [
    # No-topic science questions (from session about Indian scientists)
...
def _best_topic_from_signals(
    tags: list[str], text: str, exclude: str
) -> str | None:
...
def _infer_primary(tags: list[str], text: str) -> str | None:
    """Infer primary topic for questions with no topic yet."""
    # Try text keywords first (more reliable than tags alone for primary)
    for keywords, topic in KEYWORD_TOPIC:
...
def _assign_topics(q: KVizzingQuestion) -> KVizzingQuestion:
    ...
    if override is not None:
        ...
        if secondary_str:
            try:
                ...
                pass
        updated_q = q.question.model_copy(update={"topics": new_topics})
        return q.model_copy(update={"question": updated_q})

    # --- Already has 2+ topics: skip ---
    if len(topics) >= 2:
        return q
    ...
    # --- Find secondary ---
    secondary_str = _best_topic_from_signals(tags, text, primary.value)
    if secondary_str:
        try:
            secondary = TopicCategory(secondary_str)
            updated_q = q.question.model_copy(update={"topics": [primary, secondary]})
            return q.model_copy(update={"question": updated_q})
        except ValueError:
            pass

    # Ensure primary is written even if no secondary found
    if not topics or topics[0] != primary:
        updated_q = q.question.model_copy(update={"topics": [primary]})
        return q.model_copy(update={"question": updated_q})
...
def main() -> None:
    ...
    log.info("  Total: %d", len(all_questions))

    enriched = [_assign_topics(q) for q in all_questions]

    no_topic   = [q for q in enriched if not q.question.topics]
    one_topic  = [q for q in enriched if len(q.question.topics) == 1]
    two_topics = [q for q in enriched if len(q.question.topics) >= 2]
    log.info("  No topic:        %d", len(no_topic))
    log.info("  Primary only:    %d", len(one_topic))
    log.info("  Primary+secondary: %d", len(two_topics))

    primary_dist: Counter = Counter()
    secondary_dist: Counter = Counter()
    for q in enriched:
        t = q.question.topics
        if t:
            primary_dist[t[0].value] += 1
        if len(t) >= 2:
            secondary_dist[t[1].value] += 1

    log.info("  Primary distribution:")
    for t, n in primary_dist.most_common():
        log.info("    %s: %d", t, n)
    log.info("  Secondary distribution:")
    for t, n in secondary_dist.most_common():
        log.info("    %s: %d", t, n)

    if args.dry_run:
        log.info("Dry run — not writing.")
        log.info("Sample assignments:")
